...
namespace CloudPad
{
    ...
    static class Util
    {
        ...
        private static readonly MethodInfo _run;
        public static void RunAndWait(string path, string[] args)
        {
            var run = _run ?? _util?.GetMethod("Run",
                BindingFlags.Static | BindingFlags.Public,
                null,
                new Type[] { typeof(string), Type.GetType("LINQPad.QueryResultFormat, LINQPad"), typeof(string[]) },
                null
            );

            var exec = run?.Invoke(null, new object[] { path, 0, args });

            if (exec != null)
            {
                exec.GetType().GetMethod("Wait", BindingFlags.Public | BindingFlags.Instance)?.Invoke(exec, null);
            }
        }
    }
